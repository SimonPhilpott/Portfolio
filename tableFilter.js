...
(function scanTable() {
    ...
    for (var i in filterArray) { // Runs a check for a Document title and description column
        ...
        window.resetURLFilter = "?OpenDocument&" + filterArray.join('&');
        break;
    }
})();
...
function contentFilter(filterID, filterType) {
    ...
    showHideFilterBtn.setAttribute("style", "width:170px; margin-bottom:5px;");
    showHideFilterBtn.setAttribute("onclick", "toggleFilterButton(this); showHideDocFilter()");
    showHideFilterBtn.innerHTML = "Show the table filters<span></span>";
    ...
    // Create a show hide buttons
    var openClose = document.createElement('a');
    openClose.setAttribute("class", "button blue");
    openClose.innerText = "Show the table filters";
    ...
    for (var j = 1, row; row = docTable.rows[j]; j++) { // loop through all table rows, except for the table header
        ...
        for (var l = 0; l < columnArray.length; ++l) {
            //console.log("make = " + columnArray[l] + " in" + categoryContainer[l+1]);
    ...
    for (var m = 0; m < categoryContainer.length; ++m) {
        ...
        if (categoryContainer[m].id == "RegionCheckboxes") {
            ...
            var sortOrder = ["AmericaContainer", "UK_and_IrelandContainer", "EuropeContainer", "AfricaContainer", "Middle_EastContainer", "AsiaContainer", "AustraliaContainer"];
            for (var p = 0; p < regionNodes.length; ++p) {
                if (regionNodes[p].id == "GlobalContainer") {
                    if (regionNodes[p].id != "AmericaContainer") {
                        makeCheckbox("America", categoryContainer[m], false)
                    }
                    ...
                    if (regionNodes[p].id != "AustraliaContainer") {
                        makeCheckbox("Australia", categoryContainer[m], false)
                    }
                }
            }
            ...
            var sortedDates;
        if (categoryContainer[m].id == "DatesCheckboxes") {
    ...
    function overpopulateRegionSector(theElements) {
        for (var i = 0; i < theElements.length; ++i) {
            ...
            if (theElements[i].className.indexOf("Global") > -1) {
                theElements[i].className += " America UK_and_Ireland Europe Africa Middle_East Asia Australia";
            }
        }
    }
...
function toggleFilterButton(el) {
    if (el.className == "button cyan upArrow") {
        ...
        el.innerHTML = "Show the table filters<span></span>";
    } else {
        el.className = "button cyan upArrow";
        el.innerHTML = "Hide the table filters<span></span>";
    }
}
